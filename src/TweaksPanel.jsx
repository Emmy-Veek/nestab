/* TweaksPanel — ES module version for production extension */
...
const __TWEAKS_STYLE = `
  ...
  /* Tweaks panel open button */
  .twk-open-btn{position:fixed;right:16px;bottom:16px;z-index:2147483645;
    width:32px;height:32px;border-radius:8px;border:0;cursor:default;
    background:rgba(250,249,247,.9);color:#29261b;
    -webkit-backdrop-filter:blur(12px);backdrop-filter:blur(12px);
    border:.5px solid rgba(255,255,255,.6);
    box-shadow:0 1px 0 rgba(255,255,255,.5) inset,0 4px 12px rgba(0,0,0,.14);
    display:grid;place-items:center;font-size:14px}
  .twk-open-btn:hover{background:rgba(255,255,255,.95)}
`;

// ── useTweaks — persists via localStorage ──────────────────────────────────
...
export function TweaksPanel({ title = 'Tweaks', children }) {
  ...
  return (
    <>
      ...
      {!open && (
        <button className="twk-open-btn" title="Open Tweaks" onClick={() => setOpen(true)}>⚙</button>
      )}
      {open && (
